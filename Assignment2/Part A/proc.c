...
// Define the state variable for Xorshift RNG
// static uint rand_state;

// // Seed the random number generator
// void srand(uint seed) {
//     if (seed == 0)
//         seed = 1;  // Avoid a zero seed, as it produces non-random results
//     rand_state = seed;
// }
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      ticket_count += p->tickets;
      if(ticket_count < golden_ticket) continue;
